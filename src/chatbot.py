...
def get_weather(loc):
    ...
    return dict(summary=weather_info['summary'],
...
def parse_location(text):
    ...
    location = None
    # Simple algorithm here. Split the text on 'weather in'. If we find it,
    # take the right side.
    # If we don't find it split on 'weather' and take the left side
    # for location.
    # If we don't find a location,  throw an exception and handle it in the
    # caller.
...
def message():
    if request.method == 'POST':
        ...
        if vals['action'] == 'join':
            ...
            response_manager.add_response('text', "Hello {}!".
